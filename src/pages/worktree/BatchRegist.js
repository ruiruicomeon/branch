...
import styles from './BatchRegist.less';
...
const { Search } = Input;
const CreateFormJidi = Form.create()(props => {
    const {
        handleModalVisibleJidi,
        getScondNameListJidi,
        modalVisibleJidi,
        selectcustomJidi,
        ischecked,
        base_jidi,
        handleScondNameClickJidi,
        handleAddJidi,
    } = props;
    ...
    return (
        <Modal
            ...
            visible={modalVisibleJidi}
            // onOk={okHandle}
            footer={null}
            onCancel={() => handleModalVisibleJidi()}
            width="50%"
        >
            <div className={styles.flex_item_style}>
                {isNotBlank(getScondNameListJidi) &&
                    getScondNameListJidi.length > 0 &&
                    getScondNameListJidi.map(scondname => {
                        return (
                            <span
                                key={scondname.id}
                                className={styles.Flex_item}
                                style={
                                    isNotBlank(base_jidi) &&
                                    base_jidi.length > 0 &&
                                    base_jidi.some(item => item.id == scondname.id)
                        ...
                        return (
                            <span
                                ...
                                style={
                                    isNotBlank(zrdata) &&
                                    zrdata.length > 0 &&
                                    zrdata.some(item => item.id == scondname.id)
                                        ? // scondname.id == scondnameData.id
                                          {
                                              backgroundColor: '#40a9ff',
                                              padding: '5px 8px',
                        ...
                        return (
                            <span
                                ...
                                style={
                                    isNotBlank(zrdatazr) &&
                                    zrdatazr.length > 0 &&
                                    zrdatazr.some(item => item.id == scondname.id)
                                        ? // scondname.id == scondnameData.id
                                          {
                                              backgroundColor: '#40a9ff',
                                              padding: '5px 8px',
                        ...
                        return (
                            <span
                                ...
                                style={
                                    isNotBlank(zrdatagj) &&
                                    zrdatagj.length > 0 &&
                                    zrdatagj.some(item => item.id == scondname.id)
                                        ? // scondname.id == scondnameData.id
                                          {
                                              backgroundColor: '#40a9ff',
                                              padding: '5px 8px',
...
class Abnormal_form extends PureComponent {
    ...
    componentDidMount() {
        const { dispatch, batchregist_list } = this.props;
        ...
        dispatch({
            ...
            type: 'dictionaryL/fetch',
            payload: {
                // label:batchregist_list.map(item=>item.buCode),
                type: 'failure',
                current: 1,
                pageSize: 5000,
            },
        ...
        //  oid
        // if (location.query && location.query.id) {  // todo...
    ...
    handleSubmit = e => {
        const { dispatch, form, location, batchregist_list } = this.props;
        const {
            addfileList,
            mentionsNameArray,
            mentionsIdArray,
        ...
        form.validateFieldsAndScroll((err, val) => {
            if (!err) {
                ...
                if (
                    isNotBlank(value) &&
                    isNotBlank(value.exceptionInfo) &&
                    isNotBlank(mentionsIdArray) &&
                    mentionsIdArray.length > 0
                ) {
                    for (
                        let i = 0;
                ...
                // if (
                //     batchregist_list &&
                //     batchregist_list.length > 0 &&
                //     zrdataerji.length > 0 &&
                //     zrdatacj.length > 0
                // ) {
                //     for (let i = 0; i < batchregist_list.length; i++) {
                //         if (
                //             batchregist_list[i].winfo &&
                //             !batchregist_list[i].winfo.some(
                //                 (item, index, arr) => item.scname == zrdataerji[0].name
                //             ) &&
                //             !batchregist_list[i].winfo.some(
                //                 (item, index, arr) => item.wcode == zrdatacj[0].code
                //             )
                //         ) {
                //             message.warning(
                //                 `${batchregist_list[i].orderSelfNum}订单下的${zrdataerji[0].name}不存在,${zrdatacj[0].code}不存在   `
                //             );
                //             return;
                //         } else if (
                //             batchregist_list[i].winfo &&
                //             !batchregist_list[i].winfo.some(
                //                 (item, index, arr) => item.sccode == zrdataerji[0].code
                //             ) &&
                //             batchregist_list[i].winfo.some(
                //                 (item, index, arr) => item.wcode == zrdatacj[0].code
                //             )
                //         ) {
                //             message.warning(
                //                 `${batchregist_list[i].orderSelfNum}订单下的${zrdataerji[0].name}不存在`
                //             );
                //             return;
                //         } else if (
                //             batchregist_list[i].winfo &&
                ...
                //         ) {
                //             message.warning(
                //                 `${batchregist_list[i].orderSelfNum}订单下的${zrdatacj[0].code}不存在`
                //             );
                //             return;
                //         }
                //     }
    ...
    handleModalVisibleJidi = flag => {
        const { base_jidi } = this.state;
        let newobj = this.deepCopy(base_jidi);
        this.setState({
            modalVisible: !!flag,
            zrdata: newobj,
        });
    ...
    render() {
        ...
        const parentMethodJidi = {
            handleAddJidi: this.handleAddJidi,
            handleModalVisibleJidi: this.handleModalVisibleJidi,
            getScondNameListJidi: baseNameList, // 数据
            modalVisibleJidi,
            ischecked,
            base_jidi,
            handleScondNameClickJidi: this.handleScondNameClickJidi,
        };
        ...
        return (
            <PageHeaderWrapper
                content={
                    ...
                    <div>
                        <Breadcrumb>
                            {isNotBlank(batchregist_list) && batchregist_list.length > 0
                                ? batchregist_list.map(item => (
                                      <Breadcrumb.Item
                                          style={{ fontWeight: 'blod', color: '#000' }}
                                      >
                                          {item.orderSelfNum}
                                      </Breadcrumb.Item>
                                  ))
                            ...
                            <Breadcrumb.Item style={{ fontWeight: 'blod', color: '#000' }}>
                                {isNotBlank(batchregist_list) &&
                                isNotBlank(batchregist_list.deptName)
                                    ? batchregist_list.deptName
                                    : ''}
                            </Breadcrumb.Item>
                        </Breadcrumb>
                    </div>
            ...
            >
                    ...
                    <Form onSubmit={this.handleSubmit} style={{ marginTop: 8 }} layout="horizontal">
                        {/* <FormItem label="订单Guid" style={{ display: 'none' }}>
                            {getFieldDecorator('oid', {
                                initialValue:
                                    isNotBlank(batchregist_list) && batchregist_list.length > 0
                                        ? batchregist_list.map(item => item.oid).join(',')
                                        : '',
                                rules: [
                                    {
                                        required: true,
                        ...
                        <FormItem label="订单自编号" style={{ display: 'none' }}>
                            {getFieldDecorator('orderSelfNum', {
                                initialValue:
                                    isNotBlank(batchregist_list) && batchregist_list.length > 0
                                        ? batchregist_list.map(item => item.orderSelfNum)
                                        : '',
                                rules: [
                                    {
                                        required: true,
                        ...
                        {/* <FormItem label="Wcc编号" style={{ display: 'none' }}>
                            {getFieldDecorator('wccNum', {
                                initialValue:
                                    isNotBlank(batchregist_list) &&
                                        batchregist_list.length > 0
                                        ? batchregist_list.map(item => item.wccNum)
                                        : '',
                                rules: [
                                    {
                                        required: true,
                        ...
                        {/* <FormItem label="品类名称" style={{ display: 'none' }}>
                            {getFieldDecorator('buName', {
                                initialValue:
                                    isNotBlank(batchregist_list) &&
                                        batchregist_list.length > 0
                                        ? batchregist_list.map(item => item.buName)
                                        : '',
                                rules: [
                                    {
                                        required: false,
                        ...
                        <FormItem label="品类编码" style={{ display: 'none' }}>
                            {getFieldDecorator('buCode', {
                                initialValue:
                                    isNotBlank(batchregist_list) &&
                                        batchregist_list.length > 0
                                        ? batchregist_list.map(item => item.buCode)
                                        : '',
                                rules: [
                                    {
                                        required: false,
                        ...
                        {/* <FormItem {...formItemLayout} label="基地">
                            <Input
                                value={
                                    isNotBlank(base_jidi) && base_jidi.length > 0
                                        ? base_jidi[0].name
                                        : []
                                }
                                onClick={this.showJidi}
                            />
                        </FormItem> */}
                        ...
                        <FormItem {...formItemLayout} label="车间">
                            ...
                            {/* <Select
                                ...
                                // value={isNotBlank(newselectgjbm) && newselectgjbm.length > 0 ? newselectgjbm :
                                //     newselectzrbm &&　newselectzrbm.length > 0 ? newselectzrbm :[]
                                // }
                                value={isNotBlank(selectercj) && selectercj.length > 0 ? selectercj.map(res => res.id) :[]}
                                mode="multiple"
                                allowClear={false}
                                removeIcon={function () {
                                    return ''
                                }}
                                onDropdownVisibleChange={this.showMcj}
                            >
                                { isNotBlank(getWorkShopList)&& getWorkShopList.length &&
                                    getWorkShopList.map((item, index) => {
                                        return (
                        ...
                        <Button
                            ...
                            style={{ marginBottom: '20px', marginLeft: '230px' }}
                        >
                            选择话术
                        </Button>
                        ...
                        <FormItem>
                            ...
                            <Row>
                                ...
                                <Col offset={3} span={24}>
                                    <Upload
                                        fileList={
                                            this.state.showimgsrc.length > 0
                                                ? this.state.showimgsrc
                                                : []
                                        }
                ...
                <CreateFormJidi {...parentMethodJidi} modalVisibleJidi={modalVisibleJidi} />
