   ...
   return (
      ...
      >
         <div style={{ marginBottom: '10px' }}>
            <Search size="default" placeholder="请输入部门名称" onSearch={onDutySearch} enterButton allowClear />
         </div>
         <div className={styles.flex_item_style}>
...
@connect(({ dictionaryL, loading, sysdeptList, sysoffice, warning, sysuser, communication, speaklist }) => ({
   ...
   submitting: loading.effects['warning/recordOnecomm'],
}))
@Form.create()
class ErrorCommunication extends PureComponent {
   state = {
   ...
   componentDidMount() {
      ...
      dispatch({
         ...
         payload: { type: 'duty', current: 1, pageSize: 1000 },
      });
         ...
         dispatch({
            ...
            callback: data => {
               ...
               dispatch({
                  ...
                  payload: {
                     grade: 3,
                     // SiteCode: data && data.code ? data.code : '',
                     type: 1,
                     current: 1,
                     pageSize: 1000,
                     buCode: data.buCode,
               ...
               if (location.query.msg == 'comm') {
                  dispatch({
                     ...
                     callback: data => {
                        let personlist = [];
                        let tableData = data;
                        if (isNotBlank(getSingleofId) && isNotBlank(getSingleofId.respUser) && isNotBlank(tableData) && isNotBlank(tableData.list) && tableData.list.length > 0) {
   ...
   handleSubmit = e => {
      const { dispatch, form, getSingleofId } = this.props;
      ...
      form.validateFieldsAndScroll((err, val) => {
         if (!err) {
            ...
            if (isNotBlank(value.deptName) && isNotBlank(value.deptName.value) && isNotBlank(value.deptName.label)) {
            ...
            if (isNotBlank(value.secondCategoryName) && isNotBlank(value.secondCategoryName.value) && isNotBlank(value.secondCategoryName.label)) {
            ...
            if (selectzr.length == 0 && !isNotBlank(getSingleofId.respType)) {
               ...
               value.respType = isNotBlank(selectzr) && selectzr.length > 0 ? selectzr.map(item => item.value).join(',') : isNotBlank(getSingleofId.respType) ? getSingleofId.respType : '';
            }
            if (selectzrbm.length == 0 && selectget.length == 0 && !isNotBlank(getSingleofId.respDept)) {
               message.warning('责任部门必填!');
               return;
            } else {
               value.respDept = isNotBlank(selectzrbm) && selectzrbm.length > 0 ? selectzrbm.map(item => item.value).join(',') : isNotBlank(selectget) && selectget.length > 0 ? selectget.join(',') : isNotBlank(getSingleofId.respDept) ? getSingleofId.respDept : '';
            }

            if (selectgjbm.length == 0 && selectzrgj.length == 0 && selectget.length == 0 && !isNotBlank(getSingleofId.followDept)) {
            ...
            if (isNotBlank(location.query) && location.query.msg === 'comm' && isNotBlank(location.query.id)) {
               values.respUserQ = respUserStr || '';
            ...
            } else if (isNotBlank(location.query) && location.query.msg === 'colse' && isNotBlank(location.query.id)) {
               // 关闭异常
               values.exceptionId = location.query.id;
               values.status = 1;
               dispatch({
   ...
   handlePersonnelStandardTableChange(pagination, filtersArg, sorter) {
      // 二级弹框表格事件
   }
   ...
   render() {
      ...
      const createdFaceModalProps = {
         ...
         handleStandardTableChange: this.handlePersonnelStandardTableChange.bind(this),
      };
            ...
            content={
               ...
               <Breadcrumb>
                  <Breadcrumb.Item style={{ fontWeight: 'blod', color: '#000' }}>
                     {isNotBlank(warningDateil) && isNotBlank(warningDateil.orderSelfNum)
                        ? warningDateil.orderSelfNum
                        : isNotBlank(AddOnerecord) && isNotBlank(AddOnerecord.orderSelfNum)
                        ? AddOnerecord.orderSelfNum
                        : isNotBlank(getSingleofId) && isNotBlank(getSingleofId.orderSelfNum)
                        ? getSingleofId.orderSelfNum
                        : ''}
                  </Breadcrumb.Item>
                  <Breadcrumb.Item style={{ fontWeight: 'blod', color: '#000' }}>{isNotBlank(AddOnerecord) && isNotBlank(AddOnerecord.deptName) ? AddOnerecord.deptName : isNotBlank(getSingleofId) && isNotBlank(getSingleofId.deptName) ? getSingleofId.deptName : ''}</Breadcrumb.Item>
               </Breadcrumb>
            }
         >
            <Card bordered={false}>
               <Form onSubmit={this.handleSubmit} style={{ marginTop: 8 }} layout="horizontal">
                  <FormItem label="订单Guid" {...formItemLayout} style={{ display: 'none' }}>
                     {getFieldDecorator('oid', {
                        initialValue: isNotBlank(warningDateil) && isNotBlank(warningDateil.oid) ? warningDateil.oid : isNotBlank(AddOnerecord) && isNotBlank(AddOnerecord.oid) ? AddOnerecord.oid : isNotBlank(getSingleofId) && isNotBlank(getSingleofId.oid) ? getSingleofId.oid : '',
                        rules: [
                           {
                              required: true,
                              message: '此字段为必填字段',
                  ...
                  <FormItem label="订单自编号" {...formItemLayout} style={{ display: 'none' }}>
                     {getFieldDecorator('orderSelfNum', {
                        initialValue: isNotBlank(getSingleofId) && isNotBlank(getSingleofId.orderSelfNum) ? getSingleofId.orderSelfNum : '',
                        rules: [
                           {
                              required: true,
                              message: '此字段为必填字段',
                  ...
                  <FormItem label="Wcc编号" {...formItemLayout} style={{ display: 'none' }}>
                     {getFieldDecorator('wccNum', {
                        initialValue: isNotBlank(getSingleofId) && isNotBlank(getSingleofId.wccNum) ? getSingleofId.wccNum : '',
                        rules: [
                           {
                              required: true,
                              message: '此字段为必填字段',
                     ...
                     <FormItem label="二级品类" style={{ display: 'none' }}>
                        {getFieldDecorator('secondCategoryName', {
                           initialValue:
                              isNotBlank(getSingleofId) && isNotBlank(getSingleofId.secondCategoryName) && isNotBlank(getSingleofId.secondCategoryCode)
                                 ? {
                  ...
                  <FormItem {...formItemLayout} label="车间" style={{ display: 'none' }}>
                     {getFieldDecorator('deptName', {
                        initialValue:
                           isNotBlank(getSingleofId) && isNotBlank(getSingleofId.deptName) && isNotBlank(getSingleofId.deptCode)
                              ? {
                  ...
                  <FormItem label="基地名称" {...formItemLayout} style={{ display: 'none' }}>
                     {getFieldDecorator('siteName', {
                        initialValue: isNotBlank(getSingleofId) && isNotBlank(getSingleofId.siteName) ? getSingleofId.siteName : '',
                     })(<Input placeholder="请输入基地名称" />)}
                  </FormItem>
                  <FormItem label="基地编码" {...formItemLayout} style={{ display: 'none' }}>
                     {getFieldDecorator('siteCode', {
                        initialValue: isNotBlank(getSingleofId) && isNotBlank(getSingleofId.siteCode) ? getSingleofId.siteCode : '',
                     })(<Input placeholder="请输入基地编码" />)}
                  </FormItem>

                  <Button className={styles.button_person} type="primary" size="large" onClick={e => this.showMhs(e)} style={{ marginBottom: '8px' }}>
                     选择话术
                  </Button>
                  ...
                  <FormItem {...formItemLayout} label="责任部门">
                     <Select
                        dropdownClassName={styles.select_none}
                        value={isNotBlank(selectzrbm) && selectzrbm.length > 0 ? selectzrbm.map(res => res.id) : isNotBlank(selectget) && selectget.length > 0 ? selectget : isNotBlank(getSingleofId) && isNotBlank(getSingleofId.respDept) ? getSingleofId.respDept.split(',') : []}
                        mode="multiple"
                        allowClear={false}
                        removeIcon={function() {
                           return '';
                     ...
                     <Select
                        ...
                        value={
                           isNotBlank(selectgjbm) && selectgjbm.length > 0
                              ...
                              : isNotBlank(selectget) && selectget.length > 0
                              ? selectget
                              : isNotBlank(getSingleofId) && isNotBlank(getSingleofId.followDept)
...
export default ErrorCommunication;
