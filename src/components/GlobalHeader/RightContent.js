...
export default class GlobalHeaderRight extends PureComponent {
  ...
  render() {
    const {
      ...
      onNoticeClear,
      theme,
      TodoMatterinfoList,  // 待办数据
      communicationLoading, // loading
      pageTabs = true,
      reloadTab = true,
    } = this.props;
    const todoCountTotal = isNotBlank(TodoMatterinfoList) && isNotBlank(TodoMatterinfoList.pagination) && isNotBlank(TodoMatterinfoList.pagination.total) ? TodoMatterinfoList.pagination.total : 0;
    ...
    return (
      <div className={className}>
        ...
        <NoticeIcon
          ...
          count={currentUser.notifyCount}   // 所有的通知
          onItemClick={(item, tabProps) => {
            console.log(item, tabProps); // eslint-disable-line
            this.changeReadState(item, tabProps);
          }}
          loading={communicationLoading}  // 加载转圈
          locale={{
        ...
        <HeaderDropdown overlay={menu}>
          <span className={`${styles.action} ${styles.account}`}>
            <Avatar
              ...
              alt="avatar"
            />
            <span className={styles.name}>{currentUser.name ? currentUser.name : '未登录'}</span>
          </span>
        </HeaderDropdown>
        {/* ) : (
