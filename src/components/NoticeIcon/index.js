...
class NoticeIcon extends PureComponent {
  ...
  onItemClick = (item, tabProps) => {
    const { id } = item
    router.push(`/worktree/Communication?id=${id}`);
      ...
      return (
        <TabPane tab={tabTitle} key={title}>
          <List 
            ...
            onClick={item => this.onItemClick(item, child.props)}   // 点击每个item触发
  ...
  render() {
    const { className, count, popupVisible, bell , list} = this.props;
